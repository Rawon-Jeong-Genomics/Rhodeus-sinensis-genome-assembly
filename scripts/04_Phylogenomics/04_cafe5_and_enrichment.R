#!/usr/bin/env Rscript
# ==============================================================================
# Description: Custom GO Enrichment analysis using clusterProfiler
#              (Integrates eggNOG and InterProScan annotations for strictness)
# Software: R, clusterProfiler, pacman, tidyverse, GO.db
# ==============================================================================
...
# ==============================================================================
# 1. Annotation files
EGGNOG_FILE <- "species_eggnog.emapper.annotations"
INTERPRO_FILE <- "species_interpro.tsv"

# 2. Target gene lists (e.g., from CAFE5 results)
EXPANDED_GENES_FILE <- "species_expanded_genes.txt"
CONTRACTED_GENES_FILE <- "species_contracted_genes.txt"
...
# ==============================================================================
cat("Step 1: Parsing annotation files and constructing consensus background...\n")

# 1.1 Parse eggNOG annotations
emapper <- read_delim(EGGNOG_FILE, delim = "\t", comment = "#", col_names = FALSE, show_col_types = FALSE)
...
interpro <- read.delim(INTERPRO_FILE, header = FALSE, sep = "\t", quote = "")
...
# ==============================================================================
cat("Step 2: Performing GO Enrichment Analysis...\n")
...
run_enrichment <- function(gene_list_file, output_prefix) {
  ...
  if (!file.exists(gene_list_file)) {
    cat("⚠️ File not found:", gene_list_file, "- Skipping...\n")
    return(NULL)
  }
...
run_enrichment(EXPANDED_GENES_FILE, paste0(PREFIX, "_GO_expanded"))
run_enrichment(CONTRACTED_GENES_FILE, paste0(PREFIX, "_GO_contracted"))
